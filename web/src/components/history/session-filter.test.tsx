...
describe('SessionFilter', () => {
  it('renders search input and filter chips', () => {
    ...
    expect(screen.getByTestId('status-chip-dead')).toBeInTheDocument();
  ...
  it('accepts custom status and provider options', () => {
    render(
      <SessionFilter onFilter={vi.fn()} statusOptions={['running']} providerOptions={['codex']} />,
    );
    expect(screen.getByTestId('status-chip-running')).toBeInTheDocument();
    expect(screen.getByTestId('provider-chip-codex')).toBeInTheDocument();
    expect(screen.queryByTestId('status-chip-completed')).not.toBeInTheDocument();
  });
});
