...
- `inks` are universal named roles. Each ink can set: `description`, `provider`, `model`, `mode`, `guard_preset`, and `instructions`. Instructions are passed as system prompt for providers that support it (Claude), or prepended to the prompt for others (Codex, Gemini, OpenCode).
