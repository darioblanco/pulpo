...
- `[knowledge]` — knowledge extraction and sync
...
- `[knowledge]` configures the git-backed knowledge repo. Set `remote` to a git URL for cross-node sync. Set `inject = false` to disable automatic context injection at session spawn. Knowledge is stored at `<data_dir>/knowledge/` as JSON files committed to a local git repo.
