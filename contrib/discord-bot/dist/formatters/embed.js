...
export function sessionEmbed(session) {
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: 'Status', value: session.status, inline: true }, { name: 'Provider', value: session.provider, inline: true }, { name: 'ID', value: `\`${session.id}\``, inline: true });
    if (session.model) {
    ...
    const prompt = session.prompt.length > 200 ? session.prompt.slice(0, 200) + '...' : session.prompt;
    embed.addFields({ name: 'Prompt', value: prompt, inline: false });
    embed.setTimestamp(new Date(session.created_at));
    return embed;
}
export function eventEmbed(event) {
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: 'Status', value: event.status, inline: true }, { name: 'Node', value: event.node_name, inline: true });
    if (event.previous_status) {
        embed.addFields({ name: 'Previous', value: event.previous_status, inline: true });
    }
    if (event.output_snippet) {
        const snippet = event.output_snippet.length > 1000
...
export function personaListEmbed(personas) {
...
export function sessionListEmbed(sessions) {
