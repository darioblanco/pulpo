...
use crate::schedule::{ConcurrencyPolicy, WorktreeConfig};
...
mod tests {
    ...
    fn test_create_schedule_request_minimal() {
        let json = r#"{"name":"nightly","cron":"0 2 * * *","workdir":"/repo","prompt":"Review"}"#;
        let req: CreateScheduleRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.name, "nightly");
        assert_eq!(req.cron, "0 2 * * *");
        assert_eq!(req.workdir, "/repo");
        assert_eq!(req.prompt, "Review");
        assert!(req.provider.is_none());
        assert!(req.mode.is_none());
        assert!(req.concurrency.is_none());
        assert!(req.max_executions.is_none());
        assert!(req.worktree.is_none());
    }

    #[test]
    fn test_create_schedule_request_full() {
        let json = r#"{"name":"review","cron":"0 3 * * *","workdir":"/repo","prompt":"Review","provider":"claude","mode":"autonomous","guard_preset":"strict","model":"opus","allowed_tools":["Read"],"system_prompt":"Be thorough","persona":"reviewer","concurrency":"replace","max_executions":50}"#;
        let req: CreateScheduleRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.provider, Some(crate::session::Provider::Claude));
        assert_eq!(req.mode, Some(SessionMode::Autonomous));
        assert_eq!(
            req.concurrency,
            Some(crate::schedule::ConcurrencyPolicy::Replace)
        );
        assert_eq!(req.max_executions, Some(50));
        assert_eq!(req.model, Some("opus".into()));
    }

    #[test]
    fn test_create_schedule_request_missing_required() {
        let json = r#"{"name":"test","cron":"* * * * *"}"#;
    ...
    // --- UpdateScheduleRequest tests ---

    #[test]
    fn test_update_schedule_request_empty() {
        let json = "{}";
    ...
    fn test_update_schedule_request_partial() {
        let json = r#"{"cron":"0 4 * * *","concurrency":"allow"}"#;
        let req: UpdateScheduleRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.cron, Some("0 4 * * *".into()));
        assert_eq!(
            req.concurrency,
            Some(crate::schedule::ConcurrencyPolicy::Allow)
        );
        assert!(req.workdir.is_none());
    }
    ...
    // --- ListExecutionsQuery tests ---

    #[test]
    fn test_list_executions_query_default() {
        let q = ListExecutionsQuery::default();
        assert!(q.limit.is_none());
    }

    #[test]
    fn test_list_executions_query_with_limit() {
        let json = r#"{"limit":50}"#;
        let q: ListExecutionsQuery = serde_json::from_str(json).unwrap();
        assert_eq!(q.limit, Some(50));
    }

    #[test]
    fn test_list_executions_query_debug() {
        let q = ListExecutionsQuery { limit: Some(20) };
        let debug = format!("{q:?}");
