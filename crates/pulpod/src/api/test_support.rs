...
//! `StubBackend` → `SessionManager` → `AppState` builder. This module
//! centralizes it: [`test_state`] for the common case, [`test_state_with`]
//! for per-test `Config` tweaks (via a mutator closure), and
//! [`test_server`] / [`test_server_with`] for full-router `TestServer`
//! integration tests.
//!
...
/// vs `AppState::with_event_tx` themselves).
pub async fn test_parts() -> (Config, SessionManager, Store) {
    test_parts_with(|_| {}).await
}

/// Like [`test_parts`], but runs `mutate` on the default `Config` before the
/// `SessionManager` is built from it.
pub async fn test_parts_with(mutate: impl FnOnce(&mut Config)) -> (Config, SessionManager, Store) {
...
/// `StubBackend`-driven `SessionManager` with no stale grace period, and a
/// minimal `test-node` `Config`.
pub async fn test_state() -> Arc<AppState> {
    test_state_with(|_| {}).await
}

/// [`test_state`] with a mutator applied to the `Config` before the
/// `SessionManager` is built (see [`test_parts_with`]).
pub async fn test_state_with(mutate: impl FnOnce(&mut Config)) -> Arc<AppState> {
    let (config, manager, store) = test_parts_with(mutate).await;
