...
pub struct CommandQueue {
    queues: Arc<RwLock<HashMap<String, VecDeque<WorkerCommand>>>>,
}
...
impl CommandQueue {
    ...
    pub async fn enqueue(&self, node_name: &str, command: WorkerCommand) {
    ...
    pub async fn drain(&self, node_name: &str) -> Vec<WorkerCommand> {
        let mut queues = self.queues.write().await;
        queues
            .get_mut(node_name)
...
mod tests {
    ...
    fn stop_cmd(id: &str, session_id: &str) -> WorkerCommand {
    ...
    fn create_cmd(id: &str, name: &str) -> WorkerCommand {
    ...
    async fn test_enqueue_and_drain() {
        ...
        match &commands[1] {
            WorkerCommand::CreateSession { command_id, .. } => assert_eq!(command_id, "c2"),
            WorkerCommand::StopSession { .. } => panic!("expected CreateSession"),
        }
        match &commands[2] {
            WorkerCommand::StopSession { command_id, .. } => assert_eq!(command_id, "c3"),
