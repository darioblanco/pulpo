...
`make docs`/`make docs-serve` need mdBook — installed by `make setup`, or standalone:
...
│   │   │   ├── lock.rs           # Single-instance advisory flock on pulpod.lock
...
│   │   ├── daemon_log.rs         # Reads recent daemon log lines for CLI error hints
