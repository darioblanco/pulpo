...
- **TDD**: write failing test, implement, refactor, verify coverage — for pure logic. Every documented user-facing behavior (session lifecycle, harness adapters, watchdog, schedules, worktrees) needs a scenario test in `crates/pulpo-e2e/tests/scenarios.rs` instead (16 tests today, `s1`-`s16`, against a real `pulpod` + real tmux server + `fake-claude`/`fake-codex`/`fake-pi` — binaries scripted by `FAKE_AGENT_SCENARIO` that play each harness's CLI/hook surface; run with `make e2e` — see CLAUDE.md's "Testing" section); a `MockBackend`-driven flow test is not a substitute for new work.
