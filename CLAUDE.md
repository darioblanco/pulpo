...
- **Database**: SQLite via `sqlx`. Versioned schema migrations live in `crates/pulpod/migrations/`; `store/mod.rs` contains only the runtime store API plus a narrow legacy bridge for pre-migration databases. Use `sqlx::query!` macro for compile-time checked queries when possible.
